...
def generate_plots():
    ...
    ax6.barh(top_10_weighted_directors['primaryName'], top_10_weighted_directors['weighted_rating'], color='orange')
...
def main():
    ...
    if st.button("Recommander"):
        ...
        # Sélectionner et afficher trois graphiques de manière aléatoire
        plots = generate_plots()
        selected_plots = random.sample(plots, 3)  # Sélectionne trois graphiques au hasard

        st.write("Voici trois graphiques choisis au hasard :")
        for fig in selected_plots:
            st.pyplot(fig)
